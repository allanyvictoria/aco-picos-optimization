%% @file main_max.m
% @brief Script principal do ACO para busca do máximo da função Picos.
%
% @details
...
% o ponto de máximo global da função Picos no domínio [-3,3] x [-3,3].
%
% O algoritmo funciona da seguinte forma:
%   1. Cada formiga começa em um ponto aleatório do grid
%   2. A formiga dá nSteps passos, movendo-se para vizinhos com base
%      no feromônio (tau) e na heurística (eta)
%   3. O melhor ponto visitado por cada formiga é registrado
%   4. Feromônio é depositado nos pontos visitados, proporcional ao custo
%   5. O feromônio evapora a cada iteração
%
% A heurística eta é definida como model.Cost deslocado para valores positivos,
% atraindo as formigas para regiões de maior valor da função.
%
...
% Heurística: desloca os custos para positivo para atrair formigas
% para regiões de maior valor da função Picos
eta = model.Cost - min(model.Cost) + 1;
...
% Melhor solução global inicializada com -inf (busca máximo)
BestSol.Cost = -inf;
...
for it = 1:MaxIt
    ...
    for k = 1:nAnt
        ...
        for l = 1:nSteps
            ...
            % Atualiza o melhor ponto da formiga (busca máximo)
            if model.Cost(prox) > ant(k).BestCost
              ant(k).BestCost = model.Cost(prox);
            endif
        end

        % O custo da formiga é o maior valor encontrado no caminho
...
title('Convergência do ACO - Busca do Máximo');
grid on;
